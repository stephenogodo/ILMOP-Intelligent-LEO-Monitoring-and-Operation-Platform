    ...
    try:
        while True:
            ...
            if published % (manager.satellite_count * 10) == 0:
                ...
                # Always show both battery and temperature — both matter for
                # fault detection. Also show the fault-specific primary indicator.
                if fault == FaultType.THERMAL_RUNAWAY:
                    status = f"batt={first.battery_pct:.1f}% temp={first.temperature_c:.1f}°C"
                elif fault == FaultType.SAFE_MODE_TRIGGER:
                    status = f"batt={first.battery_pct:.1f}% safe_mode={first.safe_mode}"
                else:
                    status = f"batt={first.battery_pct:.1f}% temp={first.temperature_c:.1f}°C"
                log.info(
